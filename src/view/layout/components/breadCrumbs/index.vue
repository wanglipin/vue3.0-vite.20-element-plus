<template>
  <el-breadcrumb separator="/" style="padding: 10px 0;">
    ...
    <el-breadcrumb-item v-for="breadcrumbItem in breadcrumbList" :key="breadcrumbItem.path">
      <el-dropdown v-if="breadcrumbItem.children.length" @command="handleBreadcrumb">
        <span class="el-dropdown-link">
          {{ breadcrumbItem.meta.title }}<i class="el-icon-arrow-down el-icon--right"></i>
        </span>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item v-for="item in breadcrumbItem.children" :key="item.path" :command="item.path">{{ item.meta.title }}</el-dropdown-item>
          </el-dropdown-menu>
        </template>
      </el-dropdown>
...
export default defineComponent ({
  ...
  setup() {
    ...
    const breadcrumbList = computed(() => {
      ...
      return !router.currentRoute.value.matched[0].meta.title ? [router.currentRoute.value.matched[1]] : router.currentRoute.value.matched
    })
